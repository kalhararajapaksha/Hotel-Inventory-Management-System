...
function LoadAllExternalOrders() {
    var settings = {
    ...
    $.ajax(settings).done(function (response) {
        ...
        var ojson = JSON.parse(response.LoadAllExternalOrdersResult);
...
function SetExternalOrder() {
    var itemID = $('#drpItem').val();
    var req_Qty = $('#txtRequestQuentity').val();
    var price = $('#txtPricePerUnit').val();



    $.ajax({
        type: 'POST',
        url: "http://localhost:44372/Service.svc/ExternalOrders",
        "data": JSON.stringify({
            "oREF_ExternalOrder": {
                "OrderID": 0,
                "ItemID": itemID,
        ...
        success: function (oResponse) {
            ...
            var ojson = JSON.parse(oResponse.ExternalOrdersResult);
            console.log(ojson)
            if (ojson.Success) {
                LoadAllExternalOrders() 
                swal(ojson.Message, {
